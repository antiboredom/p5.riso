...
class ChannelMapper {
  ...
  weightedColorDistance(color1, color2) {
    return Math.sqrt(
      Math.pow(color1[0] - color2[0], 2) +
        Math.pow(color1[1] - color2[1], 2) +
        Math.pow(color1[2] - color2[2], 2),
  ...
  rgbToLab(r, g, b) {
    let xyz = this.rgbToXyz(r, g, b);
    return this.xyzToLab(xyz[0], xyz[1], xyz[2]);
  }

  deltaE(lab1, lab2) {
    // Compute the Euclidean distance between two LAB colors
  ...
  perceptualColorDistance(rgb1, rgb2) {
    let lab1 = this.rgbToLab(rgb1[0], rgb1[1], rgb1[2]);
    let lab2 = this.rgbToLab(rgb2[0], rgb2[1], rgb2[2]);
  ...
  cosineSimilarity(vec1, vec2) {
    ...
    let magnitudeVec1 = Math.sqrt(
      vec1.reduce((sum, val) => sum + val * val, 0),
    );
    let magnitudeVec2 = Math.sqrt(
      vec2.reduce((sum, val) => sum + val * val, 0),
    );
    return 1 - dotProduct / (magnitudeVec1 * magnitudeVec2);
  }
  ...
  findClosestColor(searchColor) {
    ...
    for (let color of this.colorCombos) {
      let distance = this.colorDistance(searchColor, color[0]);
      // let distance = this.cosineSimilarity(searchColor, color[0]);
      // let distance = this.perceptualColorDistance(searchColor, color[0]);
      if (distance < minDistance) {
        minDistance = distance;
        closestColor = color;
      }
    }
    this.map[searchColor.join("")] = closestColor;
    return closestColor;
  }
...
function extractMappedChannels(img, steps = 0.1) {
  const p = _getP5Instance();
  const channelImages = Riso.channels.map((_) =>
    p.createImage(img.width, img.height),
  );
  // const channelColors = Riso.channels.map((c) => c.channelColor);
  channelImages.forEach((c) => {
    c.loadPixels();
  });
  const mapper = new ChannelMapper(steps);
  img.loadPixels();
  for (let i = 0; i < img.pixels.length; i += 4) {
    ...
    for (let j = 0; j < opacities.length; j++) {
      // channelImages[j].pixels[i] = channelColors[j][0];
      // channelImages[j].pixels[i + 1] = channelColors[j][1];
      // channelImages[j].pixels[i + 2] = channelColors[j][2];
...
function testMapChannels(img) {
  const p = _getP5Instance();

  let newimg = p.createImage(img.width, img.height);
  ...
  for (let i = 0; i < img.pixels.length; i += 4) {
    ...
    newimg.pixels[i] = newc[0];
    newimg.pixels[i + 1] = newc[1];
    newimg.pixels[i + 2] = newc[2];
